...
int RBFTest_8(RecordBasedFileManager *rbfm) {
    ...
    prepareRecord(recordDescriptor.size(), nullsIndicator, 8, "UCSCSluf", 24, 170.1, 5000, record, &recordSize);
    ...
    // Given the rid, read the record from file
    rc = rbfm->readRecord(fileHandle, recordDescriptor, rid, returnedData);
    assert(rc == success && "Reading an updated record should not fail.");
